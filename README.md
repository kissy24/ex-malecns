...
ブラウザで動くアリの採餌・集団行動シミュレータです。餌や壁を配置して、個体の局所ルールとフェロモンから生まれる動きを観察できます。MaleCNS v1.0 と Stonkfly の実験に着想を得た、従来のハエの縮約神経回路・迷路モードも残しています。
...
- 60・120・240 匹のアリによる採餌と帰巣、フェロモンの蓄積と減衰
- 餌の追加、壁の描画、消しゴムによる環境変更（マウス・タッチ・キーボード）
- 3 環境プリセット、匂いの半減期・分泌量・探索の自由度の調整
- 回収量・運搬中の個体数・餌の残量・回収量の履歴の表示
...
最初に「アリの採餌」モードが開き、120 匹が探索を始めます。10〜30 秒ほど観察し、琥珀色のフェロモンが現れたら「壁を描く」で道を遮ってみてください。「餌場をすべて取り除く」から別の場所に餌を置けば、環境変化への反応を試せます。「匂いをすべて消す」は既存の場を消去し、分泌量を 0 にすると新たな匂いが出なくなります。同じ SEED・パラメータ・操作タイミングなら同じ経過になります。個体数の変更・プリセット選択・リセットは世界を初期化します。別モードや非表示タブでは進行を休止します。アリの実験状態は端末に保存しません。
...
- `lib/ants.ts`: アリの移動、局所感知、採餌、フェロモン、環境編集
